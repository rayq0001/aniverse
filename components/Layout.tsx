...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-700 ease-in-out ${isDark ? 'bg-[var(--bg-amoled)] text-white' : 'bg-white text-slate-900'} font-['Cairo']`} dir={dir}>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-center md:justify-between">
          ...
          <div className="hidden md:flex items-center gap-1 flex-1">
            {[
              ...
              { to: '/bookmarks', icon: <BookOpen size={18} /> },
              { to: '/explore', icon: <SlidersHorizontal size={18} /> },
            ].map(item => (
              <Link 
                key={item.to} 
                to={item.to} 
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            ...
            <div className="col-span-2 md:col-span-1 space-y-4">
              ...
              <div className="flex items-center gap-2">
                {/* TikTok icon removed as requested */}
            ...
            <div>
              ...
              <ul className={`space-y-2.5 text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-500'}`}>
                ...
                <li><Link to="/bookmarks" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{t('library')}</Link></li>
                <li><Link to="/explore" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{language === 'ar' ? 'البحث المتقدم' : 'Advanced Search'}</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className={`space-y-2.5 text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-500'}`}>
                <li><Link to="/explore?genre=Action" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{language === 'ar' ? 'أكشن' : 'Action'}</Link></li>
                <li><Link to="/explore?genre=Fantasy" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{language === 'ar' ? 'فانتازيا' : 'Fantasy'}</Link></li>
                <li><Link to="/explore?genre=Romance" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{language === 'ar' ? 'رومانسية' : 'Romance'}</Link></li>
                <li><Link to="/explore?genre=Comedy" className={`transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`}>{language === 'ar' ? 'كوميديا' : 'Comedy'}</Link></li>
              </ul>
            </div>
      ...
      <div id="layout-mobile-nav" className={`md:hidden fixed bottom-0 left-0 right-0 z-50 border-t px-4 py-2 ${isDark ? 'bg-[var(--bg-amoled)]/95 backdrop-blur-xl border-white/[0.06]' : 'bg-white/95 backdrop-blur-xl border-gray-200'}`}>
        <div className="flex items-center justify-around max-w-md mx-auto">
          {[
            { to: '/', icon: <Home size={20} />, label: t('home') },
            { to: '/bookmarks', icon: <BookOpen size={20} />, label: t('library') },
            { to: '/explore', icon: <SlidersHorizontal size={20} />, label: language === 'ar' ? 'بحث' : 'Search' },
          ].map(item => (
            ...
            <button 
              onClick={(e) => {
                if (location.pathname === '/admin') {
                  e.preventDefault();
                  window.dispatchEvent(new CustomEvent('showAdminNav'));
                } else {
                  navigate('/admin');
                }
              }} 
              className={`flex flex-col items-center gap-0.5 py-1 px-3 rounded-lg transition-colors ${location.pathname === '/admin' ? (isDark ? 'text-white' : 'text-black') : 'text-neutral-500'}`}
            >
              <Shield size={20} />
