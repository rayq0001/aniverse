...
(async ({ id, offset, limit, output }) => {
  ...
  try {
    ...
    const baseOutputDir = typeof output === 'string' && output.trim() ? output : process.cwd();
    const rootDir = path.resolve(baseOutputDir, `D_${sanitizeFileName(title)} - ${authorNames}`);
